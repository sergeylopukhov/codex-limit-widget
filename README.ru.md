...
Здесь же настраиваются действия по левому и правому щелчку в строке меню, что открывает щелчок по виджету (приложение, окно с подробными лимитами или Codex), сочетание клавиш для вызова окна с подробными лимитами из любого приложения, тихие часы и строка синхронизации в виджете. В диагностике видны источник данных, время последней успешной синхронизации и ответ CLI, если обновление не удалось. Кнопка `Reset data` с подтверждением удаляет локальный снимок лимитов, данные виджета, историю уведомлений и настройки приложения; учётная запись Codex и вход в CLI не затрагиваются.
