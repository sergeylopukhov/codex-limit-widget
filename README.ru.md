...
- Текущий план Codex: `Pro 5x`, `Pro 20x` или `Plus` вместо служебного идентификатора.
- Статистику использования: токены, день с наибольшим расходом, последний день, серию дней и самый долгий запрос.
- Цвет остатка в процентах по уровню: обычный, предупреждение ниже 50%, критический ниже 20%.
- Время последней успешной синхронизации в виджете; строку включает и выключает настройка.
- График расхода токенов за семь дней с подписями дат в большом бежевом виджете. Если свежие данные относятся к текущему дню по UTC, виджет подписывает их `TODAY` вместо `LAST DAY`.
...
Индикатор в строке меню показывает подробные лимиты или компактный процент. Действия по левому и правому щелчку настраиваются отдельно: контекстное меню, всплывающее окно, настройки или Codex. В контекстном меню можно обновить лимиты, скопировать статус и выйти из приложения; пока идёт обновление, видно, что данные загружаются.

Во всплывающем окне видны доступные лимиты, время сброса, свежесть данных и настройки; кнопка `Copy status` копирует план, процент и время сброса каждого лимита текстом. Когда выходит новая версия, рядом со значением появляется стрелка, а во всплывающем окне — карточка обновления.
...
Настройки разбиты на вкладки: «Общие», «Строка меню», «Виджеты», «Уведомления», «Обновления» и «Диагностика». Окно можно перетаскивать, и оно больше не висит поверх других приложений. В настройках выбираются оформление окна и режим строки меню. Если доступны оба лимита, здесь же выбирается источник процента для компактного индикатора. В режиме процента шкалу под числом и само число можно отдельно окрасить по остатку лимита: зелёный при полном лимите, тёмно-красный у нуля. Цветная шкала включена по умолчанию, цветные цифры выключены. В разделе `Updates` указаны установленная версия, результат последней проверки и действие для обновления.

Здесь же настраиваются действия по левому и правому щелчку в строке меню, что открывает щелчок по виджету (приложение, окно с подробными лимитами или Codex), сочетание клавиш, которое открывает окно с подробными лимитами из любого приложения и закрывает его повторным нажатием, тихие часы и строка синхронизации в виджете. В диагностике видны источник данных, время последней успешной синхронизации и ответ CLI, если обновление не удалось. Кнопка `Reset data` с подтверждением удаляет локальный снимок лимитов, данные виджета, историю уведомлений и настройки приложения; учётная запись Codex и вход в CLI не затрагиваются.
...
У 5-часового и недельного лимитов свои переключатели и свои пороги в настройках: каждый лимит предупреждает на выбранном для него уровне. В тихие часы такие предупреждения не приходят; сообщение о восстановлении лимита они не отключают.

Уведомление о восстановлении относится только к тому исчерпанию, которое приложение успело зафиксировать: нужно, чтобы лимит был записан на 100%, а приложение работало, когда квота снова стала доступна. Ещё нужно разрешение на уведомления в macOS: приложение запрашивает его один раз и использует для этого сообщения.
...
Если доступна новая версия, уведомление появится в строке меню, во всплывающем окне и в настройках. После нажатия `Update now` загрузится официальный ZIP для macOS. Перед установкой проверяются контрольная сумма SHA-256 из ответа GitHub, идентификатор пакета, версия и подпись кода. После проверки текущая копия в `Applications` заменяется, затем автоматически открывается новая версия. При обновлении старое расширение виджета перезапускается, чтобы оно не сохраняло прежнее действие по клику, а после обновления открывается окно `Что нового` с заметками новой версии, включая заметки 1.2.400, которые пропустило прошлое обновление.
